function Hud() {
  ...
  */
  ...
  this.render = function () {
    var scroreString = '' + score;
    var x = (width - scroreString.length * (size + padding)) / 2;
    var digitPos = createVector(x, padding);
    for (var i = 0; i < scroreString.length; i++) {
      var dmap = digitMaps[scroreString.charAt(i)];
  ...
  function drawDigit(digitMap, index, pos) {
    ...
    for (var i = 0; i < digitMap.length; i++) {
      if (digitMap[i] === true) drawLine(i, pos);
    }
    pop();
  }
...
// Hud.prototype = Object.create(Entity.prototype);
